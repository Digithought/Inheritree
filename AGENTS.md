...
- Public API: `insert`, `updateAt`, `deleteAt`, `upsert`, `merge`, `find`, `get`, `at`, `first`, `last`, `next`/`prior`, `moveNext`/`movePrior`, `ascending`/`descending` (no-arg walks the whole tree), `range`, `entries`/`keys`/`[Symbol.iterator]` (aliasing-free entry/key iteration — the safe reading default), `clear` (empty in place), `getCount`, `isValid`. All complexity O(log n); `getCount` is computed, not stored.
